...
class Queens:
            ...
            if self.piece in white_pieces:
                if self.y + c < 8 and self.x + c < 8:
                    if not_blocked0 and board[self.y + c][self.x + c] not in white_pieces:
                        queen_legal_moves.append((self.y + c, self.x + c))
                ...
                if self.y - c > -1 and self.x - c > -1:
                    ...
                    if board[self.y - c][self.x - c] in white_pieces or board[self.y - c][self.x - c] in black_pieces:
                        not_blocked3 = False
                if self.y + c < 8:
                    if not_blocked4 and board[self.y + c][self.x] not in white_pieces:
                        queen_legal_moves.append((self.y + c, self.x))
                    if board[self.y + c][self.x] in white_pieces or board[self.y + c][self.x] in black_pieces:
                        not_blocked4 = False
                if self.y - c > -1:
                    if not_blocked5 and board[self.y - c][self.x] not in white_pieces:
                        queen_legal_moves.append((self.y - c, self.x))
                    if board[self.y - c][self.x] in white_pieces or board[self.y - c][self.x] in black_pieces:
                        not_blocked5 = False
                if self.x - c > -1:
                    if not_blocked6 and board[self.y][self.x - c] not in white_pieces:
                        queen_legal_moves.append((self.y, self.x - c))
                    if board[self.y][self.x - c] in white_pieces or board[self.y][self.x - c] in black_pieces:
                        not_blocked6 = False
                if self.x + c < 8:
                    if not_blocked7 and board[self.y][self.x + c] not in white_pieces:
                        queen_legal_moves.append((self.y, self.x + c))
                    if board[self.y][self.x + c] in white_pieces or board[self.y][self.x + c] in black_pieces:
                        not_blocked7 = False
                ...
                if self.y - c > -1 and self.x - c > -1:
                    ...
                    if board[self.y - c][self.x - c] in white_pieces or board[self.y - c][self.x - c] in black_pieces:
                        not_blocked3 = False
                if self.y + c < 8:
                    if not_blocked4 and board[self.y + c][self.x] not in black_pieces:
                        queen_legal_moves.append((self.y + c, self.x))
                    if board[self.y + c][self.x] in white_pieces or board[self.y + c][self.x] in black_pieces:
                        not_blocked4 = False
                if self.y - c > -1:
                    if not_blocked5 and board[self.y - c][self.x] not in black_pieces:
                        queen_legal_moves.append((self.y - c, self.x))
                    if board[self.y - c][self.x] in white_pieces or board[self.y - c][self.x] in black_pieces:
                        not_blocked5 = False
                if self.x - c > -1:
                    if not_blocked6 and board[self.y][self.x - c] not in black_pieces:
                        queen_legal_moves.append((self.y, self.x - c))
                    if board[self.y][self.x - c] in white_pieces or board[self.y][self.x - c] in black_pieces:
                        not_blocked6 = False
                if self.x + c < 8:
                    if not_blocked7 and board[self.y][self.x + c] not in black_pieces:
                        queen_legal_moves.append((self.y, self.x + c))
                    if board[self.y][self.x + c] in white_pieces or board[self.y][self.x + c] in black_pieces:
                        not_blocked7 = False
    ...
    def getOutOfCheck(self, board, get_original_moves, all_pieces, white_pieces, black_pieces, white_piece_objects,
                      black_piece_objects):
        new_moves = []
